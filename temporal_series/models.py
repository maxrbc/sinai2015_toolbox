...
from .src.table_utils import correct_timepoint_for_negatives as  correct
...
class Graph(models.Model):
    ...
    def get_graph(self, timevar_type, na_complition):
        ...
        if timevar_type == '0':
            if min(table.get_table()[self.timevar]) < 0:
                table.get_table()[self.timevar+"_corrected"] = correct(table.get_table()[self.timevar])

            timevar, new_table = convert(table.get_table().copy(), self.timevar+"_corrected")
